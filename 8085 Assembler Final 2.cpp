...
int ProcessMOType1_Pass2( struct DEFP1OUTPUT P1OutputInst, struct MOT MOTInst[], FILE *P2OutputFP )
{
    ...
    MOTIndex = MOTGet_Pass2( MOTInst, ConcatenatedString, P1OutputInst );  // Sending the Mnemonic for searching process ( Searching only on Mnemonic )
    // Writing the processed output to the P2OutputFP File
...
struct DEFP1OUTPUT GetP1OutputInst_Pass2( FILE *P1OutputFP )
{
    ...
    fscanf( P1OutputFP, "%u", &P1OutputInst.LN );
...
int CheckSymbolTableForDuplicates_Pass1( void )                   // Checking symbol table for any duplicate ( ie. repeated ) symbols
{
    FILE *fp;
...
int ProcessDB_Pass1( struct PROG SourceInst, unsigned int *LC, FILE *fp2, unsigned int LN )  // Processing the PseudoOp DB
{
    ...
    //Write_Pass1_OutputToFile( SourceInst, LC, Comment, fp3 );  // Storing the result of PASS1 in a file ( Pass1Output.txt )
...
int ProcessEQU_Pass1( struct PROG SourceInst, unsigned int *LC, FILE *fp2, unsigned int LN )  //Processing the PseudoOp EQU
{
    ...
    else                                                    // else
    {
        ...
        STSTO_Pass1( fp2, SourceInst, temp );                     // store the value symbol = as it is in operand 1
...
void STSTO_Pass1( FILE *fp2, struct PROG SourceInst, unsigned int LC )  // Symbol table storage
{
...
char *SplitInstructionAndComment_Pass1( char str[], char Comment[] )  // wherever there is ; or // they will be replaced with \0 to mark the end of string thus deleting the comment
{
    int count, IsComment;
    //char str1[50];
...
struct PROG GetFields_Pass1( char Instruction[] )  // The instruction is broken down into smaller tokens for analysis and differentiating whether it is a label or mnemonic or a operand
{
